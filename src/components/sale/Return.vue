...
export default {
  beforeRouteLeave(to, form, next) {
    if (sessionStorage.getItem('orderid')!=null && sessionStorage.getItem('orderid').match(/^[a-z|A-Z]+/gi) == 'XSTHD') {
      ...
      this.$confirm('确认废弃该单据？', '废弃', {
        ...
        .then(() => {
          ...
          if (this.formorder.receipts.length > 0) {
            ...
            const _this = this
            this.axios({
              url: 'http://localhost:8088/frameproject/salereturn/update',
              method: 'post',
              params: {
                id: _this.formorder.returnId,
                type: type,
              },
              headers: {
                JWTDemo: state.userInfo.token,
